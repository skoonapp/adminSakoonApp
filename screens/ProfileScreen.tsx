...
const ProfileScreen: React.FC = () => {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsUploading(true);
        try {
            const storageRef = storage.ref(`profile_pictures/${profile.uid}`);
            const uploadTask = await storageRef.put(file);
            const photoURL = await uploadTask.ref.getDownloadURL();

            await db.collection('listeners').doc(profile.uid).update({ photoURL });
        } catch (error) {
              ...
              <div className="flex items-center gap-4">
                <div className="relative shrink-0 group">
                    ...
                    <button 
                        ...
                        className="w-16 h-16 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary-500 dark:focus-visible:ring-offset-slate-800"
                        aria-label="Change profile picture"
                    >
                        {profile?.photoURL ? (
                            <img src={profile.photoURL} alt="Profile" className="w-16 h-16 rounded-full object-cover border-2 border-white dark:border-slate-700" />
                        ) : (
                            <div className="w-16 h-16 bg-primary-100 dark:bg-primary-900/50 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-700">
